# Agent onboarding — FPMWithDevin

This file orients AI agents and developers. All application code is under **`FPM/`**. For install, env vars, and tech stack, see [FPM/README.md](FPM/README.md). For data flow, REST API, and module map, see [FPM/docs/ARCHITECTURE.md](FPM/docs/ARCHITECTURE.md).

## Working directory

Run every `python -m …` command from **`FPM/`**. Entry modules insert the project root on `sys.path` so imports like `fpm.*` and `mock_server.*` resolve.
...
Details: [FPM/docs/ARCHITECTURE.md](FPM/docs/ARCHITECTURE.md).
...
1. **ChromaDB skip-if-populated** — `build_knowledge_base()` in `fpm/knowledge/builder.py` returns early if the per-control collection already has documents. After changing `infrastructure/`, parsers, or chunking, delete `FPM/chroma_data` (or `CHROMADB_PERSIST_DIR`) or you will debug stale embeddings.
...
4. **`OPENAI_API_KEY`** required for FPM, evaluation, and MCP (see `FPM/.env.example`).
